...
}, system ? builtins.currentSystem, officialRelease ? false
...
let
  ...
  supportDarwin = genAttrs [ "x86_64" "aarch64" ]
      ...
      if hasInfix "-linux-" config then
        let
          bootstrap = import ../stdenv/linux/make-bootstrap-tools.nix {
            pkgs = import ../.. { localSystem = { inherit config; }; };
          };
        in { inherit (bootstrap) build test; }
  ...
  jobs = let
    packagePlatforms =
      if attrNamesOnly then id else release-lib.packagePlatforms;
    packageJobs = {
    ...
    mapTestOn-packages = if attrNamesOnly then
      pkgs // packageJobs
    else
      mapTestOn ((packagePlatforms pkgs) // packageJobs);
  in unionOfDisjoint nonPackageJobs mapTestOn-packages;
